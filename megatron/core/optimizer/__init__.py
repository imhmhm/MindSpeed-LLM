...
def _get_param_groups(
    ...
    muon_legacy_matched_adamw_rms: Optional[float],
    use_muon: bool = False,
) -> List[Dict]:
...
def _get_param_groups_and_buffers(
    ...
    param_groups = _get_param_groups(
        ...
        muon_legacy_matched_adamw_rms=config.muon_legacy_matched_adamw_rms,
        use_muon=(config.optimizer == 'muon-legacy'),
    )
...
def _get_megatron_optimizer_based_on_param_groups(
    ...
    if param_groups:
        if config.optimizer_cpu_offload:
            ...
            init_state_fn = None
        elif config.optimizer == 'muon-legacy':
            optimizer = Muon(
                param_groups,
                lr=config.lr,
                weight_decay=config.weight_decay,
                matched_adamw_rms=config.muon_legacy_matched_adamw_rms,
                momentum=config.muon_legacy_momentum,
                nesterov=config.muon_legacy_nesterov,
                ns_steps=config.muon_legacy_ns_steps,
