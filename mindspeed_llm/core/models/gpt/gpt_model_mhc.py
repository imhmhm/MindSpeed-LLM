...
from mindspeed_llm.tasks.models.transformer.deepseek4.mhc import hc_repeat, get_mhc_spec
from mindspeed_llm.training.utils import recompute_valid_actual_seq_len


class GPTModelMHC(MegatronCoreGPTModel):
    """
    Experimental GPTModel with hyper-connections (MHC), copied from gpt_model.py
    so the common GPTModel stays untouched; merge back once stable.
    """
    ...
    def __init__(self,
                 ...
                 hc_head_spec: Optional[ModuleSpec] = None,
                 *args,
                 **kwargs,
                 ) -> None:
        ...
        if hc_head_spec is None:
            hc_head_spec = get_mhc_spec(True)
        ...
        if self.mtp_process or (self.post_process and global_args.enable_mhc):
            # move block main model final norm here when mtp or mhc enable
...
# apply the same wrapper patches registered for GPTModel (reset-attention-mask,
# low-precision optimizer, ...) to this class, since its own __init__/forward
# copies shadow the patched parent methods
from mindspeed.patch_utils import MindSpeedPatchesManager

for _method in ('__init__', 'forward'):
    _patch = MindSpeedPatchesManager.patches_info.get(
        'megatron.core.models.gpt.gpt_model.GPTModel.{}'.format(_method))
    if _patch is not None and _patch.is_applied:
        _func = getattr(GPTModelMHC, _method)
        for _wrapper in _patch.wrappers:
            _func = _wrapper(_func)
        setattr(GPTModelMHC, _method, _func)