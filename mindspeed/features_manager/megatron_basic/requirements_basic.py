...
class RequirementsBasicFeature(MindSpeedFeature):
    ...
    def te_adaptation(self, pm, args):
        ...
        pm.register_patch(
            'transformer_engine.pytorch.distributed.CudaRNGStatesTracker', torch.nn.Module, create_dummy=True
        )
    ...
    def apex_adaptation(self, pm, args):
        ...
        from mindspeed.core.fusions.fused_layer_norm import FusedLayerNormAffineFunction
        ...
        pm.register_patch(
            'apex.normalization.fused_layer_norm.fused_layer_norm_affine', fused_layer_norm_affine, create_dummy=True
        )
        pm.register_patch('fused_layer_norm_cuda', create_dummy=True)
        pm.register_patch(
            'fused_weight_gradient_mlp_cuda.wgrad_gemm_accum_fp32', npu_matmul_add_fp32, create_dummy=True
        )
        pm.register_patch(
            'fused_weight_gradient_mlp_cuda.wgrad_gemm_accum_fp16', npu_matmul_add_fp16, create_dummy=True
        )
        pm.register_patch(
